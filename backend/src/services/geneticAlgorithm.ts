...
export function runGeneticAlgorithm(
  ...
  for (let generation = 0; generation < generations; generation++) {
    ...
    for (let i = 0; i < elitismCount; i++) {
      ...
      newPopulation.push(
        JSON.parse(JSON.stringify(scoredPopulation[i].timetable)),
      );
    }
    ...
    for (let i = 0; i < parents.length; i += 2) {
      ...
      if (Math.random() < mutationRate) {
        ...
        const randomCourseId =
          child1.assignments[
            Math.floor(Math.random() * child1.assignments.length)
          ].courseId;
        ...
        for (let j = 0; j < child1.assignments.length; j++) {
          if (child1.assignments[j].courseId === randomCourseId) {
            const newRoomId = roomChanged
              ? child1.assignments[j].roomId
              : randomRoom.id;
            roomChanged = true;

            child1.assignments[j] = {
              ...child1.assignments[j],
